...
\begin{itemize}[align=left, labelwidth=\widthof{4)}, labelsep=1ex, labelindent=0pt, leftmargin=\labelsep+\labelindent+\labelwidth]
  \item[1)]  Gas: Expansionsarbeit  $\delta W = - F \dd{X} = -\frac{F}{A} A \dd{x} = - p \dd{V}$ mit Druck \fbox{$p=\frac{F}{A}$}, Kraft  des Systems auf Wand. $\delta W > 0$, wenn $\dd{V} > 0$ \input{5/Graph4.tex}
...
\begin{itemize}[align=left]
  \item[\uline{Beweis}:] \begin{itemize}[align=left]
    \item[1)] Betrachte Energieänderung $\delta E^{\norBra{1}}$ bei Einführen und Variation einer Nebenbedingung\\ \input{5/Graph13.tex}
...
\begin{table}[H]
  ...
  \begin{tabular}{c | c}
    ...
    \input{5/Graph14.tex} & \input{5/Graph15.tex}\\
    \multicolumn{2}{c}{\input{5/Graph16.tex}} \\
